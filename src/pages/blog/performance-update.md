...
Ironic fulfills the needs of several different use cases. From a Systems Adminsitrator just needing to deploy the same OS image to a number of machines in fairly rapid order. To software deploying specialized ramdisk iamges and configuration such as those that may be used to stand up a small private cloud. To the largest and most complex deployments where Ironic powers the management of all Bare Metal nodes and resources within an organization.
...
<br>
If your hoping a list including the node conductor can see a performance improvement, I wouldn't expect that unless we consider additional data model changes. But if you have a use case, please bring it to our attention and maybe it will inspire us!
